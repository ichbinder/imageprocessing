				...
				for (int c = 0; c < contoures.length; c++) {
					...
					if (drawPaths) {
						...
						for (int j = 1; j < contoure.getVectors().length; j++) {
							...
							g2d.draw(new Line2D.Double(offsetX + lastVecOut.x * zoom, offsetY + lastVecOut.y * zoom,
									offsetX + nextVec.x * zoom, offsetY + nextVec.y * zoom));
							lastVecOut = nextVec;
						}
					}
					// --------------------------------------------------------------------
					...
					if (drawStraightPaths) {
						...
						SortedMap<Integer, Integer> tmpData = (TreeMap<Integer, Integer>) contoure
								.getBestStraigthPath();
						...
						while (it.hasNext()) {
							...
							g2d.draw(new Line2D.Double(offsetX + contoure.getVector(hmKey).x * zoom,
									offsetY + contoure.getVector(hmKey).y * zoom,
									offsetX + contoure.getVector(hmData).x * zoom,
									offsetY + contoure.getVector(hmData).y * zoom));
					...
					if (zoom > 1) {
						for (int i = 0; i < image.getHeight(); i++) {
							for (int j = 0; j < image.getWidth(); j++) {
								...
								g2d.drawLine(offsetX + 0, (int) (offsetY + i * zoom),
										(int) (offsetX + image.getWidth() * zoom), (int) (offsetY + i * zoom));
								g2d.drawLine((int) (offsetX + j * zoom), offsetY + 0, (int) (offsetX + j * zoom),
										(int) (offsetY + image.getHeight() * zoom));
